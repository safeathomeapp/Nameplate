...
def drawPlateTrue(self, context):
    ...
    # Ends (left/right) — robust across Blender versions:
    # Match original intent: bevel ONLY the two outer upright edges of the end cap.
    # Do NOT bevel the full outer face perimeter, because that changes the cap thickness.
    def _mark_outer_profile_edges(obj, sign):
        me = obj.data
        bm = bmesh.new()
        bm.from_mesh(me)
        ...
        # Outer face = furthest face away from the plate along local X
        target_face = max(bm.faces, key=_face_center_x) if sign > 0 else min(bm.faces, key=_face_center_x)

        # On that face, pick ONLY the two mostly-vertical edges.
        # This preserves thickness/depth and changes only the visible end profile.
        vertical_edges = []
        for e in target_face.edges:
            ...
            # Edge belongs to the end face already, so dx should be ~0.
            # We want edges that run in Z, not in Y.
            if dz > dy and dz >= dx:
                vertical_edges.append(e)

        # Fallback guard: if Blender topology is odd, choose the two face edges with biggest Z span.
        if len(vertical_edges) != 2:
            face_edges = list(target_face.edges)
            face_edges.sort(key=lambda e: abs(e.verts[0].co.z - e.verts[1].co.z), reverse=True)
            vertical_edges = face_edges[:2]
    ...
    def _apply_end_style(obj, style):
        if style in {"PLAIN", None, ""}:
            return

        mod = obj.modifiers.new(name="EndBevel", type='BEVEL')
        mod.limit_method = 'WEIGHT'
        mod.affect = 'EDGES'
        mod.use_clamp_overlap = True

        # Match original operator feel as closely as possible, but only on the two profile edges.
        if style == "BEVEL":
            mod.width = 1.85688
            mod.segments = 10
            mod.profile = 0.5
    ...
    for _side, sign, name in ends_def:
        ...
        # Mark only the outer face edges for beveling, then apply the bevel.
        _mark_outer_profile_edges(c, sign)
        _apply_end_style(c, my_main_ends)
...
class OBJECT_PT_NamePlate(Panel):
    ...
    def draw_header(self, context):
        self.layout.label(text="Name Plate Maker v1.15.1", icon='WORDWRAP_ON')
