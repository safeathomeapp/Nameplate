...
def _find_outer_face_by_x_local(bm, sign):
    if not bm.faces:
        return None

    def _face_center_x_local(face):
        return sum(vert.co.x for vert in face.verts) / len(face.verts)

    return max(bm.faces, key=_face_center_x_local) if sign > 0 else min(bm.faces, key=_face_center_x_local)

def _get_face_profile_edge_by_vertical_side_local(face, side="TOP"):
    if face is None:
        return None

    face_z_values = [vert.co.z for vert in face.verts]
    if not face_z_values:
        return None

    if side == "BOTTOM":
        target_z = min(face_z_values)
        sort_reverse = False
    else:
        target_z = max(face_z_values)
        sort_reverse = True

    z_tol = max(max(face_z_values) - min(face_z_values), 1.0) * 1e-5
    ...
    for edge in face.edges:
        ...
        # On the X-end face of TopBit, the profile edges run along local Y.
        if not (dy > dz and dy >= dx):
            continue

        both_on_target = abs(v0.z - target_z) <= z_tol and abs(v1.z - target_z) <= z_tol
        edge_mid_z = (v0.z + v1.z) * 0.5
        candidate_edges.append((both_on_target, edge_mid_z, edge))

    if not candidate_edges:
        return None

    candidate_edges.sort(key=lambda item: (item[0], item[1]), reverse=sort_reverse)
    return candidate_edges[0][2]

def _bevel_top_plate_profile(obj, style, include_bottom=False):
    if not obj or obj.type != 'MESH':
        return False
    ...
    try:
        ...
        left_face = _find_outer_face_by_x_local(bm, -1)
        right_face = _find_outer_face_by_x_local(bm, 1)

        print("\n[TopBit Debug] ----------------------------------------")
        print(f"[TopBit Debug] object={obj.name!r} style={style!r} include_bottom={include_bottom}")

        target_edges = []
        for side_name, face in (("LEFT", left_face), ("RIGHT", right_face)):
            if face is None:
                print(f"[TopBit Debug] {side_name}: no end face found")
                continue

            face_center_x = sum(v.co.x for v in face.verts) / len(face.verts)
            face_z_values = [v.co.z for v in face.verts]
            print(
                f"[TopBit Debug] {side_name}: face_index={face.index} "
                f"center_x={face_center_x:.6f} z_min={min(face_z_values):.6f} z_max={max(face_z_values):.6f}"
            )
            ...
            for edge in face.edges:
                ...
                if dy > dz and dy >= dx:
                    z_mid = (v0.z + v1.z) * 0.5
                    y_running.append((edge.index, z_mid, (v0.x, v0.y, v0.z), (v1.x, v1.y, v1.z)))

            if not y_running:
                print(f"[TopBit Debug] {side_name}: no Y-running candidate edges found")
            else:
                print(f"[TopBit Debug] {side_name}: Y-running candidate edges:")
                for edge_index, z_mid, p0, p1 in sorted(y_running, key=lambda item: (item[1], item[0])):
                    print(
                        f"    edge {edge_index}: z_mid={z_mid:.6f} "
                        f"v0=({p0[0]:.6f},{p0[1]:.6f},{p0[2]:.6f}) "
                        f"v1=({p1[0]:.6f},{p1[1]:.6f},{p1[2]:.6f})"
                    )

            top_edge = _get_face_profile_edge_by_vertical_side_local(face, side="TOP")
            if top_edge is not None:
                print(f"[TopBit Debug] {side_name}: selected TOP edge={top_edge.index}")
                if top_edge not in target_edges:
                    target_edges.append(top_edge)
            else:
                print(f"[TopBit Debug] {side_name}: selected TOP edge=None")

            if include_bottom:
                bottom_edge = _get_face_profile_edge_by_vertical_side_local(face, side="BOTTOM")
                if bottom_edge is not None:
                    print(f"[TopBit Debug] {side_name}: selected BOTTOM edge={bottom_edge.index}")
                    if bottom_edge not in target_edges:
                        target_edges.append(bottom_edge)
                else:
                    print(f"[TopBit Debug] {side_name}: selected BOTTOM edge=None")

        print(f"[TopBit Debug] final target edge indices={[e.index for e in target_edges]}")

        if not _apply_bevel_to_bmesh_edges(bm, target_edges, style):
            print("[TopBit Debug] bevel helper returned False")
            return False

        print("[TopBit Debug] bevel helper returned True")
        bm.to_mesh(mesh)
        mesh.update()
        return True
    finally:
...
def drawPlateTrue(self, context):
    ...
    # Top plate position is now fixed on top.
    # TopBit is created as a cube, so its location is the centre of the mesh.
    # Keep the placement logic explicit and local to this build step:
    # - drop_top_halfway=True  -> centre of TopBit sits at the plate top plane
    #                            so the top plate drops halfway into the base.
    # - drop_top_halfway=False -> centre is raised by half its own height so the
    #                            whole TopBit sits on top of the base plate.
    drop_top_halfway = bool(getattr(bpy.context.scene.my_tool, 'drop_top_halfway', False))
    top_position = user_z if drop_top_halfway else (user_z + (top_height * 0.5))
    ...
    if bpy.context.scene.my_tool.add_top:
        ...
        try:
            _bevel_top_plate_profile(
                c,
                bpy.context.scene.my_tool.my_top_ends,
                include_bottom=bpy.context.scene.my_tool.top_bottom_match,
            )
        except Exception:
            _ensure_object_mode()
    else:
...
class MyProperties(PropertyGroup):
    ...
    top_bottom_match: bpy.props.BoolProperty(
        name="Shape Bottom Too",
        description="Apply the selected top plate end style to the bottom corners as well",
        default=False,
        update=drawPlate
    )
                ...
                if obj and obj.name == 'PLATE':
                    if import_plate:
                        ...
                        if mytool.top_options:
                            ...
                            match_label = "Shape Bottom Too" if mytool.top_bottom_match else "Bottom Square"
                            match_icon = 'TRIA_DOWN_BAR' if mytool.top_bottom_match else 'MESH_PLANE'
                            box.prop(mytool, "top_bottom_match", text=match_label, icon=match_icon, toggle=True)
