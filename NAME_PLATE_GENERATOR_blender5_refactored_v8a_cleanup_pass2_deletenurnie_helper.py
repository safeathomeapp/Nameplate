...
def _flip_nurnie(side):
    ...
    side_map = {
        "LEFT": {
            ...
            "source_name": "NUR_LEFT",
            "parent_name": "NURNIE_LEFT",
        },
        "RIGHT": {
            "unhide": unhidenurnieright,
            "source_name": "NUR_RIGHT",
            "parent_name": "NURNIE_RIGHT",
        },
    }

    config = side_map.get(side)
    if not config:
        return {'CANCELLED'}

    config["unhide"](None)

    source_obj = bpy.context.scene.objects.get(config["source_name"])
    if not source_obj:
        return {'CANCELLED'}
    ...
    parent_obj = bpy.context.scene.objects.get(config["parent_name"])
    if parent_obj:
        _deselect_all()
        _set_active(parent_obj)
        parent_obj.select_set(True)

    source_data_obj = bpy.data.objects.get(config["source_name"])
    if source_data_obj:
...
def _delete_nurnie(side):
    ...
    if side_key == 'LEFT':
        object_names = ('NURNIE_LEFT', 'NUR_LEFT')
    elif side_key == 'RIGHT':
        object_names = ('NURNIE_RIGHT', 'NUR_RIGHT')
    else:
        return {'CANCELLED'}
