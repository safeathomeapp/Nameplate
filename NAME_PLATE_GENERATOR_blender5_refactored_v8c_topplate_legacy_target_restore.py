...
def _bevel_top_plate_profile(obj, style):
    # Deliberately keep TopBit on legacy cube-topology targeting.
    # Unlike the main end-cap mesh, TopBit is created from a fresh primitive cube
    # immediately before shaping, so the intended top-left/top-right profile edges
    # remain the stable legacy indices 2 and 8. This avoids transform/order-
    # dependent geometric mis-selection when the top plate is repositioned or resized.
    return _bevel_mesh_edges_by_index(obj, (2, 8), style)
