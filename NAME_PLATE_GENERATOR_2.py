...
class MyProperties(bpy.types.PropertyGroup):
    ...
    bbteam_options : bpy.props.BoolProperty(name="", default=False)
...
class OBJECT_PT_NamePlate(Panel):
    ...
    def draw_header(self, context):
        layout = self.layout
        layout.label(text="Name Plate Maker v2.1.4", icon= 'WORDWRAP_ON')
        ...
        if 'PLATE' in bpy.context.scene.objects:
            ...
            if obj.name == 'PLATE':                             
                if 'IMPORTPLATE' in bpy.context.scene.objects:      
                    ...
                    box = layout.box()
                    row = box.row()   
                    row.prop(mytool, "bbteam_options")
                    row.label(text="BB Team Builder", icon='USER') 
                    if mytool.bbteam_options == True:
                        row = box.row()
                        row.label(text="Add FOV cut out", icon='LINCURVE')    
