...
def _bevel_top_plate_profile(obj, style):
    if not obj or obj.type != 'MESH':
        return False
    ...
    try:
        ...
        # TopBit is created from a fresh cube primitive. Its end-profile edges are
        # stable in local topology and confirmed by debug output:
        #   left bottom = 0, left top = 2, right bottom = 7, right top = 8
        #
        # Behaviour choice:
        # - sitting on top: bevel the original top pair only (2, 8)
        # - dropped half way: bevel the requested pair only (7, 8)
        drop_top_halfway = bool(getattr(bpy.context.scene.my_tool, 'drop_top_halfway', False))
        target_indices = (7, 8) if drop_top_halfway else (2, 8)
        target_edges = [bm.edges[i] for i in target_indices if 0 <= i < len(bm.edges)]
