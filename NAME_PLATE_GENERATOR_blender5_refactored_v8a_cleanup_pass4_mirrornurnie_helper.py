...
def _get_top_face_profile_edge(obj, face):
    ...
    for edge in face.edges:
        ...
        # Only consider the edge that runs across the plate depth (world/local Y direction).
...
def _bevel_top_plate_profile(obj, style, drop_top_halfway=False):
    ...
    try:
        ...
        # Confirmed stable TopBit cube indices from live debug output:
        # left bottom = 0
        # left top    = 2
        # right bottom= 7
        # right top   = 8
...
def drawPlateTrue(self, context):
    ...
    # - drop_top_halfway=True  -> centre TopBit on the NameplateTop band.
    #                            This means dropping it from the normal on-top
    #                            position by half TopBit height plus half the
    #                            NameplateTop height (border_thickness).
...
def _mirror_nurnie(side):
    ...
    source_side = str(side).upper()
    target_side = 'RIGHT' if source_side == 'LEFT' else 'LEFT'
    source_config = NURNIE_CONFIG.get(source_side)
    target_config = NURNIE_CONFIG.get(target_side)
    ...
    _safe_remove_object(target_config["nurnie"])
    _safe_remove_object(target_config["nur"])

    source_anchor_obj = bpy.data.objects.get(source_config["nurnie"])
    if not source_anchor_obj:
        return {'CANCELLED'}

    mirrored_y = source_anchor_obj.location.y
    mirrored_z = source_anchor_obj.location.z
    instance_scale = source_anchor_obj.instance_faces_scale

    source_config["unhide"](None)

    source_nur_obj = bpy.context.scene.objects.get(source_config["nur"])
    if not source_nur_obj:
        return {'CANCELLED'}

    _deselect_all()
    _set_active(source_nur_obj)
    source_nur_obj.select_set(True)
    bpy.ops.object.duplicate()
    bpy.ops.object.parent_clear(type='CLEAR')

    duplicated_nur_obj = bpy.context.active_object
    if not duplicated_nur_obj:
        return {'CANCELLED'}
    duplicated_nur_obj.name = target_config["nur"]

    tool = bpy.context.scene.my_tool
    circle_curve = float(tool.angles) / 100.0
    if circle_curve == 0.41:
        circle_curve = 0.415

    if base_type == "S":
        plane_x = (base_size_x - 2) if target_side == 'RIGHT' else 2
        plane_location = (plane_x, -base_size_y * 0.5 - 0.5, 2)
    else:
        curve_offset = (base_size_x * PI) * circle_curve * 0.5
        end_offset = float(tool.end_length) * 0.5
        plane_x = (base_size_x * PI) * 0.25

        if target_side == 'RIGHT':
            plane_x = plane_x + curve_offset - end_offset
        else:
            plane_x = plane_x - curve_offset + end_offset

        plane_location = (plane_x, -1.5, int(tool.my_user_z) * 0.5)
    ...
    target_anchor_obj = bpy.context.active_object
    if not target_anchor_obj:
        return {'CANCELLED'}

    target_anchor_obj.name = target_config["nurnie"]
    target_anchor_obj.location[0] = plane_x
    target_anchor_obj.location[1] = mirrored_y
    target_anchor_obj.location[2] = mirrored_z

    target_nur_obj = bpy.data.objects.get(target_config["nur"])
    target_anchor_obj = bpy.data.objects.get(target_config["nurnie"])
    if not target_nur_obj or not target_anchor_obj:
        return {'CANCELLED'}

    _deselect_all()
    _set_active(target_anchor_obj)
    target_anchor_obj.select_set(True)

    target_nur_obj.parent = target_anchor_obj
    bpy.ops.object.modifier_add(type='CURVE')
    bpy.context.object.modifiers["Curve"].object = bpy.data.objects.get("PATH")
    bpy.context.object.instance_type = 'FACES'
    bpy.context.object.show_instancer_for_render = False
    bpy.context.object.show_instancer_for_viewport = False
    bpy.context.object.use_instance_faces_scale = True
    bpy.context.object.instance_faces_scale = instance_scale
    bpy.context.object.rotation_euler[0] = -0.261799

    for config in NURNIE_CONFIG.values():
        hidden_nur_obj = bpy.data.objects.get(config["nur"])
        if hidden_nur_obj:
            hidden_nur_obj.hide_set(True)

    restored_source_anchor = bpy.context.scene.objects.get(source_config["nurnie"])
    if restored_source_anchor:
        _deselect_all()
        _set_active(restored_source_anchor)
        restored_source_anchor.select_set(True)
