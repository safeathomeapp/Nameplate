...
def _get_face_profile_edge_by_vertical_side(obj, face, side="TOP"):
    if face is None:
        return None
    ...
    if side == "BOTTOM":
        target_z = min(co.z for co in face_world)
        z_rank = min
        sort_reverse = False
    else:
        target_z = max(co.z for co in face_world)
        z_rank = max
        sort_reverse = True
    ...
    for edge in face.edges:
        ...
        edge_target_z = z_rank(v0.z, v1.z)
        edge_mid_z = (v0.z + v1.z) * 0.5
        candidate_edges.append((both_on_target, edge_target_z, edge_mid_z, edge))

    if not candidate_edges:
        return None

    candidate_edges.sort(key=lambda item: (item[0], item[1], item[2]), reverse=sort_reverse)
...
def _bevel_top_plate_profile(obj, style, include_bottom=False):
    ...
    try:
        ...
        left_face = _find_outer_face_by_x_world(obj, bm, -1)
        right_face = _find_outer_face_by_x_world(obj, bm, 1)

        target_edges = []
        for face in (left_face, right_face):
            top_edge = _get_face_profile_edge_by_vertical_side(obj, face, side="TOP")
            if top_edge is not None and top_edge not in target_edges:
                target_edges.append(top_edge)

            if include_bottom:
                bottom_edge = _get_face_profile_edge_by_vertical_side(obj, face, side="BOTTOM")
