...
def drawPlateTrue(self, context):
    ...
    def _mark_outer_profile_edges(obj, sign):
        ...
        # On that face, pick ONLY the two horizontal edges (running in local Y on the end face).
        # This preserves thickness/depth and changes the top-to-bottom profile rather than front-to-back.
        profile_edges = []
        for e in target_face.edges:
            ...
            # We want the two edges that run in Y, not Z.
            if dy > dz and dy >= dx:
                profile_edges.append(e)

        # Fallback guard: if Blender topology is odd, choose the two face edges with biggest Y span.
        if len(profile_edges) != 2:
            face_edges = list(target_face.edges)
            face_edges.sort(key=lambda e: abs(e.verts[0].co.y - e.verts[1].co.y), reverse=True)
            profile_edges = face_edges[:2]
