...
def drawPlateTrue(self, context):
    ...
    # Do NOT rely on edge indices; instead bevel only the outer end face via bevel-weight + modifier.
    def _mark_outer_end_edges(obj, sign):
        me = obj.data
        bm = bmesh.new()
        bm.from_mesh(me)
        ...
        # Pick the face farthest from the plate along local X.
        def _face_center_x(f):
            return sum(v.co.x for v in f.verts) / len(f.verts)

        target = max(bm.faces, key=_face_center_x) if sign > 0 else min(bm.faces, key=_face_center_x)
    ...
    def _apply_end_style(obj, style):
        # Style names match your EnumProperty values.
        ...
        # Keep your original numeric "feel" as close as possible.
        if style == "BEVEL":
            mod.width = 1.85688
            mod.segments = 10
            mod.profile = 0.5
...
class OBJECT_PT_NamePlate(Panel):
    ...
    def draw_header(self, context):
        self.layout.label(text="Name Plate Maker v1.15", icon='WORDWRAP_ON')
