...
def _get_top_face_profile_edge(face):
    if face is None:
        return None
    ...
    top_z = max(vert.co.z for vert in face_verts)
    z_tol = max(abs(top_z), 1.0) * 1e-6
    ...
    for edge in face.edges:
        ...
        # On the left/right end faces of TopBit, the target profile edge runs in local Y.
        if abs_y > abs_z and abs_y >= abs_x:
            y_edges.append(edge)
    ...
    for edge in y_edges:
        v0, v1 = edge.verts
        if abs(v0.co.z - top_z) <= z_tol and abs(v1.co.z - top_z) <= z_tol:
            top_plane_edges.append(edge)

    if len(top_plane_edges) == 1:
        return top_plane_edges[0]

    if len(top_plane_edges) > 1:
        top_plane_edges.sort(
            key=lambda edge: (
                (edge.verts[0].co.z + edge.verts[1].co.z) * 0.5,
                abs(edge.verts[1].co.y - edge.verts[0].co.y),
            ),
            reverse=True,
        )
        return top_plane_edges[0]
...
def _bevel_top_plate_profile(obj, style):
    ...
    try:
        ...
        left_face = _find_outer_face_by_x(bm, -1)
        right_face = _find_outer_face_by_x(bm, 1)

        target_edges = []
        for face in (left_face, right_face):
            edge = _get_top_face_profile_edge(face)
            if edge is not None and edge not in target_edges:
                target_edges.append(edge)

        # Safety net for primitive cube topology only: the intended legacy top edges
        # are stable on a freshly-added cube. Use only if geometric detection fails.
        if len(target_edges) != 2 and len(bm.edges) >= 9:
            fallback = []
            for edge_index in (2, 8):
                if 0 <= edge_index < len(bm.edges):
                    edge = bm.edges[edge_index]
                    if edge not in fallback:
                        fallback.append(edge)
            target_edges = fallback
...
def drawPlateTrue(self, context):
    ...
    if bpy.context.scene.my_tool.add_top:
        ...
        try:
            _bevel_top_plate_profile(c, bpy.context.scene.my_tool.my_top_ends)
        except Exception:
            _ensure_object_mode()
    else:
        _safe_remove_object('TopBit')
